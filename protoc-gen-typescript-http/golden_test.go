...
// golden 树(examples/proto/gen/typescript)逐字节对比。生成器输出一旦漂移即
// 失败,防止"改了生成器忘了同步示例"或"示例被手改"。buf 不在 PATH 时跳过。
func TestGoldenExamples(t *testing.T) {
	...
	goldenDir := filepath.Join(exDir, "gen", "typescript")

	bin := filepath.Join(t.TempDir(), "protoc-gen-typescript-http")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	cmdBuild := exec.Command("go", "build", "-o", bin, ".")
	cmdBuild.Env = append(os.Environ(), "GOWORK=off")
